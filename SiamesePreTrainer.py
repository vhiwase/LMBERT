...
class SiamesePreTrainer:
    def __init__(self, model: LMBERT, train_dataset: LMBERTDataset, eval_dataset: LMBERTDataset = None,
                 ...
                 model_dir: Union[str, Path] = "./LMBERT", use_tensorboard: bool = False,
                 tensorboard_log_path: str = "./tensorboard_logs", device: str = 'cpu'):
        """
        Initialize the Trainer to perform LMBERT's Siamese Pre-training

        :param model: The LMBERT model to train
    ...
    def train(self, **kwargs) -> None:
        ...
        logging.info(f"Starting LMBERT pre-training with {self.epochs} epochs and {self.num_batches} batches")
        ...
        if self.save_best_model:
            ...
            self.model = LMBERT.load(checkpoint_path)
    ...
    def evaluate(self, eval_dataset: LMBERTDataset = None) -> torch.Tensor:
        """
        ...
        base_language_sentences, target_language_sentences, labels = eval_dataset.get_batch(0, self.batch_size)
    ...
    def load_checkpoint(self, checkpoint_directory: Path) -> None:
        ...
        self.model = LMBERT.load(checkpoint_directory)
    ...
    def save_model(self, path: Union[str, Path]) -> None:
        """
        Saves the LMBERT model to the provided path by invoking the save method of the model

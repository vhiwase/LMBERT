...
class LMBERTDataset(Dataset):
    def __init__(self, base_language_sentences: List[str], target_language_sentences: List[str], labels: float = None,
        ...
        """Create a LMBERTDataset from a list of base language sentences and target language sentences.
        ...
        """
        logging.info(f"Creating LMBERTDataset")
        logging.debug(f"Creating LMBERTDataset with args: {locals()}")
    ...
    def save(self, path: Union[str, Path]) -> None:
        ...
        :return: None
        """
        logging.info(f"Saving LMBERTDataset dataset to {path}")
        torch.save(self, path)

    @staticmethod
    def load(path: Union[str, Path]) -> 'LMBERTDataset':
        """Load a LMBERTDataset object from the given path.

        :param path: Path to load the dataset object
        :return: LMBERTDataset object
        """
        logging.info(f"Loading LMBERTDataset dataset from {path}")

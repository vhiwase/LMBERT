...
class LMBERT(nn.Module):
    def __init__(self, model_path: str, num_pooling_layers: int = 1, pooling_method: str = 'mean',
                 device: str = 'cpu'):
        """Initialize LMBERT model for Siamese Pre-training.

        :param model_path: Path to the Transformer model and Tokenizer to use for LMBERT.
        ...
        """
        logging.info(
            f"Creating LMBERT model with args: {model_path}, {num_pooling_layers}, {pooling_method}, {device}")
    ...
    def add_tokens_to_tokenizer(self, tokens: List[str]) -> int:
        """Add new tokens to the LMBERT Tokenizer.
    ...
    def encode(self, sentence: str) -> Dict[str, torch.Tensor]:
        """Encode a sentence using the LMBERT Tokenizer
    ...
    def batch_encode(self, sentences: List[str]) -> Dict[str, torch.Tensor]:
        """Encode a list of sentences using the LMBERT Tokenizer.
    ...
    def save(self, path: Union[Path, str], save_pretrained: bool = True, save_tokenizer: bool = True) -> None:
        """Saves the LMBERT Siamese Network model
        ...
        :return: None
        """
        logging.info(f"Saving LMBERT model to {path}")
        save_directory = Path(path)
        if not save_directory.exists():
            save_directory.mkdir(parents=True)
        torch.save(self.state_dict(), save_directory.joinpath('LMBERT.pt'))
        if save_pretrained:
    ...
    @staticmethod
    def load(path: Union[Path, str], transformer_path: Union[str, None] = None) -> 'LMBERT':
        """Loads the LMBERT Siamese Network model.

        :param path: The path to the LMBERT model. If this is a directory, ensure that it contains the LMBERT.py file and the config.json to load the Transformer. If this is a file, it should be the LMBERT.pt file.
        :param transformer_path: The path to the Transformer model. If None, the model is loaded from the path using the config.json.
        :return: The loaded LMBERT Siamese Network model.
        """
        logging.info(f"Loading LMBERT model from {path}")
        path = Path(path)
        if path.is_dir():
